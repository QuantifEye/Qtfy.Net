...
internal partial class BigRationalTests
{
    ...
    {
        Assert.That((BigRational)left != (BigRational)right, Is.EqualTo(left != right));
    }
    ...
    {
        Assert.That(HasMethod<BigRational, BigRational, TReturn>(name), Is.True);
        Assert.That(HasMethod<BigRational, BigRational, TReturn>(name), Is.True);
        Assert.That(HasMethod<BigRational, BigInteger, TReturn>(name), Is.True);
        Assert.That(HasMethod<BigInteger, BigRational, TReturn>(name), Is.True);
        Assert.That(HasMethod<BigRational, ulong, TReturn>(name), Is.True);
        Assert.That(HasMethod<ulong, BigRational, TReturn>(name), Is.True);
        Assert.That(HasMethod<BigRational, long, TReturn>(name), Is.True);
        Assert.That(HasMethod<long, BigRational, TReturn>(name), Is.True);
    }

    [SuppressMessage("Microsoft.Performance", "CA1812", Justification = "class is instantiated by unit testing")]
    private sealed class OverloadCases : IEnumerable
    {

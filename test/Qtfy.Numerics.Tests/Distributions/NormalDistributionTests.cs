...
internal sealed class NormalDistributionTests
{
    [TestCase(2d, 0d)]
    [TestCase(2d, -1d)]
    ...
    public void TestMu()
    {
        ...
        Assert.That(distribution.Mu, Is.EqualTo(mu));
    }
    ...
    public void TestSigma(double sigma)
    {
        ...
        Assert.That(distribution.Sigma, Is.EqualTo(sigma));
        Assert.That(distribution.StandardDeviation, Is.EqualTo(sigma));
        Assert.That(distribution.Variance, Is.EqualTo(sigma * sigma));
    }

    [TestCase(1, 1, 1)]
    public void TestMean(double mu, double sigma, double expected)
    {
        Assert.That(new NormalDistribution(mu, sigma).Mean, Is.EqualTo(expected));
    }

...
internal sealed class UniformRealDistributionTests
{
    [TestCase(2d, 1d)]
    ...
    [TestCase(1.375, 1.5)]
    public void TestMin(double min, double max)
    {
        Assert.That(new UniformRealDistribution(min, max).Min, Is.EqualTo(min));
    }

    [TestCase(1.375, 1.5)]
    public void TestMax(double min, double max)
    {
        Assert.That(new UniformRealDistribution(min, max).Max, Is.EqualTo(max));
    }
    ...
    [TestCase(1.5, 3.75, double.NaN, double.NaN)]
